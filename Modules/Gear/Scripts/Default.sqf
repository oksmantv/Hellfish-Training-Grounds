...
switch (toLower(_role)) do {
	...
	case "r": {
		...
		[[_cables,1],[_defusalKit,1],[_clacker,1],[_demoCharge,2]] call _addToBackpack;
		[_map, _gps, "", _compass, _watch, ""] call _addLinkedItems;
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			_nvg call _addNVG;
	...
	case "ag": {
		...
		[[_LMG_mag_tr,COUNT_AG_MAGS(_LMG_mag_tr)],[_cables,1],[_defusalKit,1],[_clacker,1],[_demoCharge,2]] call _addToBackpack;
	...
	case "ear": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_pistol_mag,4],[_smokegrenadeY,2]] call _addToUniform;
		[[_grenade,2],[_grenademini,2]] call _addToVest;
		[_map, _gps, "", _compass, _watch, ""] call _addLinkedItems;
		if ((call EFUNC(Common,isNight)) && _allowedNightStuff) then {
			_nvg call _addNVG;
	...
	case "eag": {
		...
		[[_pistol_mag,2],[_smokegrenadeY,4],[_grenademini,2]] call _addToUniform;
		[] call _addToVest;
		[[_LMG_mag_tr,COUNT_AR_MAGS(_LMG_mag_tr)],[_cables,1],[_defusalKit,1],[_clacker,1],[_demoCharge,2]] call _addToBackpack;
		if (GVAR(extraGear)) then {
			(_FAKBig) call _addToBackpack;
		} else {
			(_FAKSmall) call _addToBackpack;
